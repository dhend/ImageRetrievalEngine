...
void ColorAutoCorrelogram::CreateColorAutoCorrelogram(const cv::Mat& img, std::vector<double>& correlogram) {
    ...
    for (int i = 0; i < img.rows; ++i) {
        ...
        for (int j = 0; j < img.cols; ++j) {
            int color_H = (int)data_H[j] / 6;
			int color_S = (int)data_S[j] / 9;
            pix_num[color_H]++;
            for (int d = 1; d <= distance; ++d) {
                int samecolor_num = 0;
                int total_num = 0;
		    ...
		    int color_H_d = ((int) color_H_uchar) / 6;
		    int color_S_d = ((int) color_S_uchar) / 9;
                    if (color_H == color_H_d && color_S == color_S_d) {
                        samecolor_num++;
                    }
                    total_num++;
                }
		    ...
		    int color_S_d = ((int) color_S_uchar) / 9;
                ...
                correlogram[(d - 1) * color_num + color_H] += (double)samecolor_num / (double)total_num;
            }
        }
    }
    for(int correlogram_row = 0; correlogram_row < distance; ++correlogram_row) {
